...
from training import training_loop_inpainting
...
@click.option('--data',          help='Path to the dataset', metavar='ZIP|DIR',                     type=str, default="./adaptsets/FFHQ_64.zip")
...
@click.option('--gpu-ids',       help='which GPU to use', metavar='LIST',                           type=parse_int_list,  default = [2])
...
@click.option('--snap',          help='How often to save snapshots', metavar='TICKS',               type=click.IntRange(min=1), default=4, show_default=True)
...
@click.option('--transfer',      help='Transfer learning from network pickle', metavar='PKL|URL',   default = './model_zoo/inpainting/ffhq.pkl', type=str)
...
@click.option('--max_gradnorm_scale',help= 'scale of max grad norm to strat  ',                          type = float,   default = 5)


# measurement related
@click.option('--img_size',                 help= 'img size',                                   type = int,     default = 64)
@click.option('--inpaint_prob',             help= 'probablity of inpainting',                   type = float,   default = 0.2)
@click.option('--inpaint_patch',            help= 'patch size',                                 type = int,     default = 16)
...
def main(**kwargs):
    ...
    c.measurement_kwargs = dnnlib.EasyDict(prob = opts.inpaint_prob, patch= opts.inpaint_patch)
    ...
    # Train.
    training_loop_inpainting.training_loop(**c)
